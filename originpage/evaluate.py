...
python evaluate.py -i ./../generated_dataset1_photons_mean_pred.hdf5 -r ./../data/dataset_1_photons_2.hdf5 -m all -d 1-photons --output_dir results_1-photons
...
if __name__ == '__main__':
    ...
    if args.mode in ['all', 'avg-E']:
        ...
        for i in range(len(target_energies)-1):
            ...
            if np.sum(which_showers) == 0:
                print("No showers in energy range [{}, {}] MeV, skipping.".format(
                    target_energies[i], target_energies[i+1]))
                continue
