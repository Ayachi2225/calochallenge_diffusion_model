...
class HighLevelFeatures:
    ...
    def _DrawShower(self, data, filename, title):
        ...
        for idx, layer in enumerate(self.relevantLayers):
            ...
            layer_data = data[layer_boundaries[idx]:layer_boundaries[idx+1]]
            if layer_data.size == 0 or np.all(layer_data <= 0):
                print(f"Layer {layer} has no energy deposition, skipping plot.")
                continue
